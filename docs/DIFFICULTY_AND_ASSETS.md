...
以 **Fairy（仙女）** 为样板；总关建议 **85**（可在区间内微调）。短包可按比例压缩到约 **30** 关（Colors / Fruits：8 + 12 + 4 + 6）。

| 段 | 建议关数 | 难度感 | 差异设计 | 网格 | FX |
|----|----------|--------|----------|------|-----|
| **简单段** | **15**（区间 10–20；短包约 8） | 轻松 | 一群同类里，有一个 **明显不一样**（剪影 / 主色整块不同，扫一眼可见） | 3×3→4×4 | 无 |
| **中等段** | **40**（区间 30–50；短包约 12） | 主力 | **同一造型体系或同色系 / 近亲水果**，Odd 仅 **某一部位或明度/邻近色 / 易错认形状** 不同 | 4×4 | 无 |
| **过渡段** | **10**（短包约 4） | 略放松 | 比中等段 **更易**，给玩家喘口气，再进高难 | 4×4 | 无 |
| **高难段** | **20**（短包约 6） | 难 | **更小细节 / 微明度 / 渐变** + 可 **移动/换位**（`teleport`） | 5×5 | teleport（建议过半开启） |

合计默认：15 + 40 + 10 + 20 = **85 关**；短包默认 **30 关**。
